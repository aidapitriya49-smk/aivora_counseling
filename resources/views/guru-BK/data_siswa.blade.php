        ...
        <table class="w-full text-left">
            ...
            <tbody class="text-sm text-slate-700">
                ...
                <tr class="border-b border-slate-50 ">
                    ...
                    <td class="py-4 px-4 text-slate-800 font-medium">{{ $siswa->nisn ?? '-' }}</td>
                   <td class="py-4 px-4 text-slate-800">{{ $siswa->tempat_tanggal_lahir ?? '-' }}</td>
                    <td class="py-4 px-4 text-slate-800">{{ $siswa->alamat ?? '-' }}</td>
                    <td class="py-4 px-4">
                       <div class="flex justify-center gap-2">
    <a href="{{ route('guru-bk.siswa_pelanggaran', $siswa->id) }}" title="Riwayat Pelanggaran" 
       class="bg-amber-500 text-white p-2 rounded-xl hover:bg-amber-600 transition shadow-sm">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
    </a>

    <a href="{{ route('guru-bk.siswa_surat', $siswa->id) }}" title="Kirim Surat WhatsApp" 
       class="bg-emerald-500 text-white p-2 rounded-xl hover:bg-emerald-600 transition shadow-sm">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
