...
<main class="mt-32 px-12 pb-20">
   <div class=" p-8 rounded-[2.5rem] shadow-sm  border-slate-100">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h2 class="text-xs font-black text-slate-700 uppercase tracking-[0.3em] mb-2">Manajemen</h2>
            <h3 class="text-2xl font-black text-slate-800 uppercase">Data Pelanggaran</h3>
        </div>
        <div class="px-4 py-2 bg-amber-50 text-amber-600 rounded-2xl text-xs font-black border border-amber-100 uppercase tracking-widest">
            Total Kasus: {{ $pelanggarans->count() }}
        </div>
    </div>
        ...
        <table class="w-full text-left">
            ...
            <tbody class="text-sm text-slate-700">
                ...
                <tr class="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    <td class="py-4 px-4 font-bold text-slate-800">{{ $p->siswa->nama_siswa ?? 'N/A' }}</td>
                    <td class="py-4 px-4 text-slate-500">{{ \Carbon\Carbon::parse($p->tanggal)->format('d M Y') }}</td>
                    <td class="py-4 px-4">{{ $p->jenis_pelanggaran }}</td>
                    <td class="py-4 px-4 text-amber-600 font-bold">{{ $p->sanksi }}</td>
                    <td class="py-4 px-4 text-center">
                        <button class="bg-amber-500 text-white px-4 py-2 rounded-xl text-[10px] font-black hover:bg-amber-600 transition">DETAIL</button>
                    </td>
                </tr>
                @endforeach
            </tbody>
